...
class CompleteEnrichmentPipeline:
            ...
            try:
                ...
                for file in patent_files[:5]:  # Limit to first 5 files for memory
                    try:
                        ...
                        org_cols = [col for col in df.columns if 'org' in col.lower() or 'assignee' in col.lower()]
                        if org_cols:
                            org_col = org_cols[0]
                            for _, row in df.iterrows():
                                org = str(row.get(org_col, '')).lower().strip()
                                if org and org != 'nan' and len(org) > 3:
    ...
    async def _enrich_single_company(self, company: pd.Series, 
        ...
        try:
            ...
            # Skip if no UEI or already processed
            if pd.isna(uei) or uei in self.processed_ueis:
                return None
                
            # Add to processed set
            self.processed_ueis.add(uei)
    ...
    async def _enrich_census(self, state: str, session: aiohttp.ClientSession) -> Dict:
        ...
        if not state or not self.census_api_key or pd.isna(state):
            return {}
    ...
    async def _enrich_fred(self, state: str, session: aiohttp.ClientSession) -> Dict:
        ...
        if not state or not self.fred_api_key or pd.isna(state):
            return {}
