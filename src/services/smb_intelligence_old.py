...
async def enrich_data():
    ...
    if not csv_files:
        print("\n⚠️  No DSBS CSV files found in current directory!")
    ...
    # Load data
    all_data = []
    for file in csv_files[:3]:  # Process first 3 files for demo
    ...
    # Enrich data (limit to first 1000 for demo)
    sample_size = min(1000, len(all_data))
    print(f"\n🔄 Enriching first {sample_size} records (for demo)...")
