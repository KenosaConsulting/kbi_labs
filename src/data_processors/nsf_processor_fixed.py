#!/usr/bin/env python3
"""
NSF Award Data Processor - Fixed Version
Correctly extracts institution names from inst field
"""
...
class NSFProcessor:
    ...
    def load_awards(self, path="/app/nsf_data", limit=None):
        ...
        for i, file in enumerate(json_files):
            ...
            try:
                with open(file, "r") as f:
                    award = json.load(f)
                    # Extract institution info from inst field
                    inst_data = award.get("inst", {})
                    if inst_data:
                        award["institution_name"] = inst_data.get("inst_name", "")
                        award["institution_city"] = inst_data.get("inst_city_name", "")
                        award["institution_state"] = inst_data.get("inst_state_code", "")
    ...
    def search_by_organization(self, org_name):
        ...
        for award in self.awards:
            ...
            if inst_name and (clean_name in inst_name or inst_name in clean_name):
                # Get PI info
                ...
                matches.append({
                    ...
                    "amount": float(award.get("awd_amount", 0)),
...
if __name__ == "__main__":
    ...
    for org in test_orgs:
        ...
        print(f"  Recent awards (2023+): {metrics['recent_awards']}")
        if metrics["awards"]:
            print(f"  Example: {metrics['awards'][0]['title']}")
            print(f"           ${metrics['awards'][0]['amount']:,.0f} ({metrics['awards'][0]['start_date']})")
